...
[![Tests: 1069 passed](https://img.shields.io/badge/tests-1069%20passed-brightgreen)]()
...
**1069 tests passed, 2 skipped** — 66 个测试文件覆盖 78 个流程节点 (77 已覆盖, 98.7%)，含主路径 + 边界 + 安全检查 + redact 对抗性测试 + 韧性层重试/熔断/降级 + 流式输出 + 子智能体派发协议 + 对话服务 + 渠道抽象。
...
- **v3.3 对话层** ✅ (#88-#89): OpenUI 风格对话页面 (/chat, 消息列表 + 输入框 + 流式渲染 + 对话历史侧栏) + ChatService (多轮对话 + 持久化 + 人格注入) + SSE 流式端点 (POST /api/chat/stream) + Gateway 渠道抽象层 (Channel 协议 + Gateway 路由 + WebChannel + 注册表, 预留微信/Telegram/Discord 扩展)
...
- 88 个 issues 全部关闭，1069 tests passed

## E2E 生产级实验

Mortis v3 在真实 minimax MiniMax-M3 API 环境下进行全项端到端（E2E）生产级测试，覆盖审计报告 §02 中全部 11 个 LLM 调用点、6 个安全机制、3 级 Dream 流水线、完整认知周期（AWAKE→REFLECT→DREAM_LIGHT）、6 个 HTML 页面 + 6 个 JSON API 端点。报告含完整调用链分析（图片呈现调用链，含文件名 + 行号标注）、信息流转模拟、Vault 写入点追踪、信号传播图、Web UI 数据流转校验。报告提供两个版本，按读者类型选择：
...
**E2E 实验摘要**：
- 38/38 步骤 100% 通过率（含原 31 步 + 新增 7 步：异常输入 + 韧性层 + 流式输出 + 子智能体派发），44 次真实 LLM 调用 + 6 次 Web 交互，总耗时 286.21s
