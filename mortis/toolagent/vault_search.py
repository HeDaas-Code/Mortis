            ...
            if query:
                ...
                for rel in rels:
                    try:
                        ...
                        # 文件不存在 (被外部删除等) — 静默跳过, 搜索结果只是少一项
                        continue
                    except VaultAccessDenied as e:
                        # issue #71 MEDIUM-D: 路径枚举攻击防护
                        # VaultAccessDenied 表示 rel_path 命中 BLOCKED_PREFIX,
                        # 必须 log warning (而非静默), 否则攻击者可通过 matches 列表
                        # 探测哪些路径被拒绝。
                        _logger.warning(
                        ...
                        continue
                    except Exception as e:  # noqa: BLE001 — 真正的未知异常, log + 跳过
                        _logger.warning(
